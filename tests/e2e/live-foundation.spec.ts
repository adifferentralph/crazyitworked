...
async function createAccount(role: "BUYER" | "SELLER", suffix: string): Promise<TestAccount> {
  ...
  const { data, error } = await client.auth.signUp({
    ...
    options: {
      data: {
        ...
        requested_role: role,
        ...(role === "SELLER" ? { store_name: `Foundation Parts ${suffix}` } : {}),
      },
    },
  });

  expect(error, `${role} signup should succeed`).toBeNull();
  expect(data.user?.id, `${role} signup should return a user`).toBeTruthy();
...
async function loginThroughUi(page: Page, account: TestAccount) {
...
test("live Supabase Auth, session, route, and RLS boundaries", async ({ page }, testInfo) => {
  test.skip(
  ...
  try {
    const buyer = await createAccount("BUYER", suffix);
    createdUserIds.push(buyer.id);

    // Supabase Auth defaults to a 60-second signup-confirmation email cooldown.
    await delay(61_000);

    const seller = await createAccount("SELLER", suffix);
    ...
    const unconfirmedClient = createPublicClient();
    const unconfirmedLogin = await unconfirmedClient.auth.signInWithPassword({
      email: buyer.email,
    ...
    await expect(page).toHaveURL(/\/login\?next=%2Faccount$/, { timeout: 30_000 });
