...
const vendorBaseUrl = process.env.PLAYWRIGHT_VENDOR_BASE_URL ?? "http://vendors.localhost:3000";
const environment = z
  .object({
  ...
  const { data, error } = await client.auth.signUp({
    ...
    options: {
      data: {
        ...
        store_name: `Phase 2 ${label} Parts`,
      },
    },
  });
...
async function waitForSeller(sql: Sql, seller: TestSeller) {
  for (let attempt = 0; attempt < 20; attempt += 1) {
    const rows =
      await sql`select user_id from public.seller_profiles where user_id = ${seller.id}::uuid`;
...
async function loginUi(page: Page, seller: TestSeller) {
  await page.goto(`${vendorBaseUrl}/login?next=/seller/dashboard`);
  const rejectOptionalCookies = page.getByRole("button", { name: "Reject optional" });
  try {
    await rejectOptionalCookies.waitFor({ state: "visible", timeout: 10_000 });
    await rejectOptionalCookies.click();
  } catch {
    // Consent was already stored for this browser context.
  }
  ...
  await expect(
    page.getByRole("heading", { level: 2, name: "Complete your supplier profile" }),
...
function productInsert(sellerId: string, categoryId: string, id = randomUUID()) {
  return {
    ...
    description:
      "A genuine live-test automotive part with a documented condition and exact identifiers.",
    id,
...
test("live seller onboarding, product media, workflow, and ownership RLS", async ({
  page,
}, testInfo) => {
  test.skip(
  ...
  try {
    const staleUserIds = (
      await sql<{ id: string }[]>`select id::text from auth.users where email like 'phase2-%'`
    ).map((row) => row.id);
    if (staleUserIds.length > 0) {
      const [staleStorage] = await sql<
        { count: number }[]
      >`select count(*)::int as count from storage.objects where bucket_id = 'product-media' and split_part(name, '/', 1) in ${sql(staleUserIds)}`;
      expect(
        staleStorage?.count,
        "stale test media must be removed through Storage before identity cleanup",
      ).toBe(0);
    ...
    expect(
      (
        await outsider.auth.signInWithPassword({
          email: sellerTwo.email,
          password: sellerTwo.password,
        })
      ).error,
    ).toBeNull();
    ...
    const incompleteInsert = await owner
      .from("products")
      .insert(productInsert(sellerOne.id, categoryId!));
    ...
    await expect(page.getByLabel("Business Registration Number")).toHaveValue("");
    ...
    await expect(page).toHaveURL(/\/dashboard\?message=onboarding-complete$/, {
      timeout: 30_000,
    });
    ...
    const approvedInsert = await owner
      .from("products")
      .insert({ ...productInsert(sellerOne.id, categoryId!), status: "APPROVED" });
    expect(approvedInsert.error, "seller cannot directly publish a new product").not.toBeNull();

    await page.goto(`${vendorBaseUrl}/products/new`);
    ...
    await page
      ...
      .fill(
        "Genuine used Toyota brake pad set inspected for the live seller workflow test. Includes visible markings and packaging evidence.",
      );
    await page.getByLabel("Your SKU").fill(`LIVE-${suffix}`);
    await page.getByLabel("Cars this part fits").selectOption([fitmentId!]);
    ...
    for (const label of [
      "Main product image",
      "Additional product angle",
      "Detail image",
      "Part-number or marking image",
      "Packaging or verification image",
    ]) {
      await page
        ...
        .setInputFiles({
          buffer: imageBytes,
          mimeType: "image/png",
          name: `${label.toLowerCase().replaceAll(/[^a-z]+/g, "-")}.png`,
        });
    }
    await page.getByRole("button", { name: "Save draft", exact: true }).click();
    await expect(page).toHaveURL(/\/products\/[0-9a-f-]+\?message=draft-created$/, {
      timeout: 90_000,
    });
    ...
    await expect(page).toHaveURL(new RegExp(`/products/${productId}\\?message=submitted$`), {
      timeout: 90_000,
    });
    console.log("[phase2-live] edited product submitted for review");

    const [databaseProduct] = await sql<
      { name: string; quantity: number; status: string }[]
    >`select name, quantity, status::text from public.products where id = ${productId}::uuid`;
    expect(databaseProduct).toMatchObject({
      name: "Toyota Camry genuine front brake pad set",
      quantity: 9,
      status: "PENDING_REVIEW",
    });
    const [mediaCount] = await sql<
      { count: number }[]
    >`select count(*)::int as count from public.product_images where product_id = ${productId}::uuid and is_active`;
    expect(mediaCount?.count).toBe(5);
    const [optionalProductData] = await sql<
      {
        crossReferences: number;
        manufacturerPartNumber: string | null;
        oemPartNumber: string | null;
        primaryFitments: number;
      }[]
    >`
      select
        products.manufacturer_part_number as "manufacturerPartNumber",
        products.oem_part_number as "oemPartNumber",
        (select count(*)::int from public.product_cross_references where product_id = products.id) as "crossReferences",
        (select count(*)::int from public.product_fitments where product_id = products.id and is_active and is_primary) as "primaryFitments"
      from public.products
      where products.id = ${productId}::uuid
    `;
    expect(optionalProductData).toEqual({
      crossReferences: 0,
    ...
    const [historyCount] = await sql<
      { inventory: number; modifications: number }[]
    >`select (select count(*)::int from public.inventory_transactions where product_id = ${productId}::uuid) as inventory, (select count(*)::int from public.product_modification_history where product_id = ${productId}::uuid) as modifications`;
    ...
    const outsiderMedia = await outsider.storage
      .from("product-media")
      .createSignedUrl(storagePaths[0]!, 60);
    expect(outsiderMedia.error).not.toBeNull();
    const outsiderUpload = await outsider.storage
      .from("product-media")
      .upload(`${sellerOne.id}/${productId}/${randomUUID()}.png`, imageBytes, {
    ...
    const draftInsert = await outsider
      .from("products")
      .insert(productInsert(sellerTwo.id, categoryId!, insufficientId));
    expect(draftInsert.error).toBeNull();
    const insufficientFitment = await outsider.from("product_fitments").insert({
      fitment_id: fitmentId!,
      is_primary: true,
      product_id: insufficientId,
    });
    expect(insufficientFitment.error).toBeNull();
    const prematureSubmit = await outsider
      .from("products")
      .update({ status: "PENDING_REVIEW" })
      .eq("id", insufficientId);
    ...
    const approvedPublicProduct = await anonymous
      .from("products")
    ...
    const publicSearch = await anonymous
      .from("marketplace_product_search")
      .select("product_id,search_text")
      .eq("product_id", productId!)
      .single();
    expect(publicSearch.error).toBeNull();
    expect(publicSearch.data?.search_text).toContain("Toyota Camry genuine front brake pad set");
    const publicSeller = await anonymous
      .from("marketplace_sellers")
      .select("seller_id,store_name")
      .eq("seller_id", sellerOne.id)
      .single();
    expect(publicSeller.error).toBeNull();
    expect(publicSeller.data?.store_name).toBe("Phase 2 Owner Parts");
    const publicMedia = await anonymous.storage
      .from("product-media")
      .createSignedUrl(storagePaths[0]!, 60);
    expect(publicMedia.error).toBeNull();
    const anonymousDraftRead = await anonymous
      .from("products")
      .select("id")
      .eq("id", insufficientId);
    expect(anonymousDraftRead.error).toBeNull();
    expect(anonymousDraftRead.data).toEqual([]);
    const outsiderApprovedUpdate = await outsider
      .from("products")
      .update({ name: "Unauthorized approved change" })
      .eq("id", productId!)
      .select("id");
    ...
    await page.goto("http://localhost:3000/find-a-part?q=Toyota%20Camry");
    await expect(
      page.getByRole("link", { name: /Toyota Camry genuine front brake pad set/i }),
    ...
    await expect(
      page.getByRole("heading", { level: 1, name: "Toyota Camry genuine front brake pad set" }),
    ).toBeVisible({ timeout: 90_000 });
    console.log(
      "[phase3-live] approved product, public search, media, and seller projection verified",
    );
  } finally {
    ...
    if (sellerOneClient && storagePaths.length > 0) {
      try {
        await sql.unsafe('drop policy if exists "phase2_test_media_cleanup" on storage.objects');
        await sql.unsafe(
          `create policy "phase2_test_media_cleanup" on storage.objects for delete to authenticated using (bucket_id = 'product-media' and (select auth.uid())::text = (storage.foldername(name))[1] and exists (select 1 from public.products where products.id::text = (storage.foldername(storage.objects.name))[2] and products.seller_id = (select auth.uid()) and products.name like 'Toyota Camry genuine%'))`,
        );
