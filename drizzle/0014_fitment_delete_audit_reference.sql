...
BEGIN
  ...
  SELECT p.id, p.seller_id INTO product_id_value, seller_id_value
  FROM public.products p WHERE p.id = source_row.product_id;
  seller_id_value := coalesce(seller_id_value, source_row.claimed_by_user_id);
  ...
  INSERT INTO public.fitment_claim_history (
    ...
    jsonb_build_object(
      'notesChanged', CASE WHEN TG_OP = 'UPDATE' THEN NEW.notes IS DISTINCT FROM OLD.notes ELSE false END,
      'sourceProductId', source_row.product_id
    )
  );
  RETURN source_row;
END;
