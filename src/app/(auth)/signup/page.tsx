...
const accountTypes = [
  {
    description: "Find compatible parts, save vehicles, and manage part requests.",
    href: "/signup/buyer",
    icon: Wrench,
    label: "I need automotive parts",
  },
  {
    description: "Create a store account and prepare to list and manage inventory.",
    href: "/signup/seller",
    icon: Store,
...
export default function SignupPage() {
  return (
    ...
    >
      <h2 className="text-3xl font-semibold text-stone-950">How will you use Twenty-Two Parts?</h2>
      <p className="mt-3 text-stone-600">Choose one account type to continue.</p>

      <div className="mt-8 grid gap-4">
        {accountTypes.map(({ description, href, icon: Icon, label }) => (
          <Link
            className="flex items-start gap-4 rounded-lg border border-stone-300 bg-white p-5 outline-none focus-visible:border-primary focus-visible:ring-2 focus-visible:ring-primary/20"
            href={href}
            key={href}
          >
            ...
            <span className="min-w-0 flex-1">
              <span className="block font-semibold text-stone-950">{label}</span>
