    ...
    <section className="min-h-[70vh] bg-white py-10 sm:py-14">
      <div className="container-page">
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-2">
          ...
          <section className="rounded-lg border border-stone-200 bg-[#fffdf9] p-6"><div className="flex items-center gap-2"><CarFront className="size-5 text-primary" aria-hidden="true" /><h2 className="text-2xl font-semibold">Verified vehicle fitment</h2></div>{fitments.length ? <ul className="mt-4 grid gap-3">{fitments.map((fitment) => <li className="flex items-start gap-2 rounded-md border border-emerald-200 bg-emerald-50 p-3 text-sm font-semibold text-emerald-900" key={fitment.id}><BadgeCheck className="mt-0.5 size-4 shrink-0" aria-hidden="true" />Fits {fitment.label}</li>)}</ul> : <p className="mt-4 text-sm leading-6 text-stone-600">No verified vehicle fitment has been attached to this listing. Match by exact part number and confirm with the supplier before ordering.</p>}</section>
        </div>
