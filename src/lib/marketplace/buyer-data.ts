...
export async function getBuyerCart(buyerId: string) {
  ...
  const { data: products } = await supabase
    .from("products")
    .select("id, slug, name, price_minor, quantity, reserved_quantity, seller_id, city, state")
    .in("id", productIds)
    .eq("status", "APPROVED");
  ...
  const [{ data: images }, { data: sellers }] = await Promise.all([
    supabase
      ...
      .eq("is_active", true)
      .eq("is_primary", true),
    supabase.from("marketplace_sellers").select("seller_id, store_name").in("seller_id", sellerIds),
  ]);
  const paths = (images ?? []).map((image) => image.storage_path);
  const { data: signed } = paths.length
    ? await supabase.storage.from("product-media").createSignedUrls(paths, 3600)
    : { data: [] };
  const signedByPath = new Map((signed ?? []).map((image, index) => [paths[index], image.signedUrl ?? null]));
  const imageByProduct = new Map((images ?? []).map((image) => [image.product_id, signedByPath.get(image.storage_path) ?? null]));
  const sellerById = new Map((sellers ?? []).map((seller) => [seller.seller_id, seller.store_name]));
...
export async function getBuyerCartCount(buyerId: string) {
  const supabase = await createClient();
  const { data } = await supabase.from("cart_items").select("quantity").eq("buyer_id", buyerId);
